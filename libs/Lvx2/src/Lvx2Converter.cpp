...
static bool savePointCloudAsPCDBinaryFile(const QString& filePath, const QVector<PointCloudPoint>& points) {
    QFile f(filePath);
                                 ...
                                 const std::function<void(int, int)>& progress)
{
    ...
    auto writePoints = [this, format](const QString& path, const QVector<PointCloudPoint>& pts) -> bool {
        switch (format) {
    ...
    if (mode == Lvx2ConvertMode::MergeAllToOne) {
        ...
        for (int i = 0; i < total; ++i) {
            ...
            while (cursor + qint64(sizeof(Lvx2PackageHeader)) <= qint64(frameIndex.nextOffset)) {
                ...
                iteratePackagePoints(packageHeader, payload, extrinsic, [&](const PointCloudPoint& p) {
                    if (pcdWriteFailed) {
                        return;
                    }
                    if (format == Lvx2ConvertFormat::CSV) {
