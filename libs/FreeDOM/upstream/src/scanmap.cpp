...
void ScanMap::build_scan_blocks(const pcl::PointCloud<pcl::PointXYZ>& cloud, const Eigen::Isometry3d& transform)
{
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(std::async(std::launch::async, [&, start, end]() {
            ...
            for(std::size_t source_point_index = start;
                source_point_index < end;
                ++source_point_index)
            {
                const pcl::PointXYZ& pcl_point = cloud[source_point_index];
                ...
                local_block.source_point_indices.emplace_back(source_point_index);
...
void ScanMap::write_point_dynamic_levels(std::size_t input_point_count,
                                         std::vector<DynamicLevel>& output,
                                         std::vector<bool>& valid) const
{
    output.assign(input_point_count, DynamicLevel::STATIC);
    valid.assign(input_point_count, false);

    for(const ScanBlock& scan_block : scan_blocks)
    {
        const LocalScanBlock& local_block = local_blocks[scan_block.local_block_linear_idx];
        for(const ScanVoxel& scan_voxel : scan_block.scan_voxels)
        {
            for(const LinearIndex local_point_index : scan_voxel.points)
            {
                const std::size_t source_point_index =
                    local_block.source_point_indices[local_point_index];
                output[source_point_index] = scan_voxel.dynamic_level;
                valid[source_point_index] = true;
            }
        }
    }
}
...
void ScanMap::reset()
{
    ...
    for(const ScanBlock& scan_block : scan_blocks)
    {
        ...
        std::vector<std::size_t>().swap(
            local_blocks[scan_block.local_block_linear_idx].source_point_indices);
    }
