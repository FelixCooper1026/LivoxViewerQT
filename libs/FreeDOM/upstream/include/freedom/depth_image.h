...
private:
    // LiDAR fov
    double lidar_horizon_fov = 0.0;
    double lidar_vertical_fov = 0.0;
    double lidar_vertical_fov_upper = 0.0;
    double lidar_vertical_fov_lower = 0.0;

    // depth image
    unsigned int rows = 0;
    unsigned int cols = 0;
    double depth_unit = 0.0;
    double depth_unit_inv = 0.0;
    double horizon_res = 0.0;
    double vertical_res = 0.0;
    double horizon_res_half = 0.0;
    double vertical_res_half = 0.0;
    double horizon_res_inv = 0.0;
    double vertical_res_inv = 0.0;

    double vertical_fov = 0.0;
    double vertical_fov_upper = 0.0;
    double vertical_fov_lower = 0.0;

    // raycast enhancement 参数
    double depth_image_min_range = 0.0;
    double max_raycast_enhancement_range = 0.0;
    unsigned int raycast_enhancement_depth_margin = 0;

    unsigned int inpaint_size = 0;
    unsigned int erosion_size = 0;
    unsigned int min_raycast_enhancement_area_pixel_num = 0;
    unsigned int top_margin = 0;

    // 全景图对应参数
    bool is_panorama = false;
    unsigned int col_margin = 0;     //根据卷积核大小得到的图像两侧区域大小，等效于图像左右连通
    unsigned int cols_w_margin = 0;
    unsigned int image_col_min = 0;  //图最小列数（不包括margin）
    unsigned int image_col_max = 0;  //图最大列数（不包括margin）
    ...
    // learn fov mask
    bool learn_fov = false;
    unsigned int scan_count = 0;
    std::unique_ptr<cv::Mat> average_image;

    // fov mask
    bool enable_fov_mask = false;
